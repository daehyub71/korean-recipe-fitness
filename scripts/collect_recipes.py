...
SERVICE_NAME = "COOKRCP01"  # 조리식품의 레시피 DB 서비스명

# 출력 디렉토리
OUTPUT_DIR = PROJECT_ROOT / "data" / "raw"
OUTPUT_FILE = OUTPUT_DIR / "recipes_raw.json"
...
def collect_all_recipes(
    ...
    logger.info(f"전체 레시피 수: {total_count}")

    if total_count == 0:
        logger.error("레시피 데이터를 가져올 수 없습니다.")
        return []
    ...
    while start_idx <= total_count:
        ...
        data = fetch_recipe_page(start_idx, end_idx)

        if data and SERVICE_NAME in data:
            rows = data[SERVICE_NAME].get("row", [])
            all_recipes.extend(rows)
            logger.info(f"  → {len(rows)}개 레시피 추가 (총 {len(all_recipes)}개)")
        else:
...
def save_recipes(recipes: list[dict], output_path: Path) -> None:
    """레시피 데이터 저장"""
...
def main():
    """메인 실행 함수"""
    logger.info("=" * 50)
    logger.info("조리식품 레시피 DB 수집 시작")
    logger.info("=" * 50)

    if not API_KEY:
        logger.error("RECIPE_API_KEY 환경변수가 설정되지 않았습니다.")
    ...
    # 레시피 수집
    recipes = collect_all_recipes(batch_size=100)

    if not recipes:
        logger.error("수집된 레시피가 없습니다.")
        sys.exit(1)
